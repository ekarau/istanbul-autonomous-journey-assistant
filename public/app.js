document.addEventListener('DOMContentLoaded', function () {
    ...
    //  1. Live fetch → IBB CKAN endpoint (via CORS proxy)
    //  2. Fallback   → assets/data/incidents-snapshot.json
    //  UI shows "Last updated: HH:MM (live)" or "(cached)"
    // ============================================================

    // IBB CKAN resource ID for "Ulaşım Yönetim Merkezi Trafik Duyuru"
    const IBB_CKAN_URL = 'https://data.ibb.gov.tr/api/3/action/datastore_search' +
                         '?resource_id=f1a5c0de-4f5b-4c6a-8e3d-9b7a2c1f0e5d&limit=50';
    const CORS_PROXY   = 'https://corsproxy.io/?';
    const SNAPSHOT_URL = 'assets/data/incidents-snapshot.json';

    // Live mutable array — tüm model fonksiyonları bunu kullanır
    let INCIDENT_ZONES = [];

    /**
     * IBB CKAN kaydını uygulama modelinin beklediği formata dönüştür.
     * Alan adları IBB şemasına göre eşleştirilmiştir.
     */
    function mapCKANRecord(record) {
        const typeMap = {
            'kaza':    { type: 'accident',  factorKey: 'accident',  radius: 3.0 },
            'yol':     { type: 'work',      factorKey: 'roadwork',  radius: 2.5 },
            'ariza':   { type: 'breakdown', factorKey: 'breakdown', radius: 1.5 },
            'hava':    { type: 'weather',   factorKey: 'rain',      radius: 5.0 }
        };
        const raw = (record.DUYURU_TURU || record.duyuru_turu || '').toLowerCase();
        const matched = Object.keys(typeMap).find(k => raw.includes(k)) || 'kaza';
        const meta = typeMap[matched];

        const lat = parseFloat(record.LATITUDE  || record.lat  || record.Y || 0);
        const lon = parseFloat(record.LONGITUDE || record.lon  || record.X || 0);

        // Koordinat geçersizse atla
        if (!lat || !lon || lat < 40.8 || lat > 41.5 || lon < 28.0 || lon > 29.5) return null;

        return {
            lat,
            lon,
            type:      meta.type,
            factorKey: meta.factorKey,
            radius:    meta.radius,
            title:     record.BASLIK || record.baslik || record.ACIKLAMA || 'Traffic Incident'
        };
    }

    /** UI'daki "Last updated" badge'ini güncelle */
    function setDataSourceBadge(isLive, dateStr) {
        const badge = document.getElementById('data-source-badge');
        if (!badge) return;
        const time = dateStr || new Date().toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' });
        if (isLive) {
            badge.innerHTML = `<span class="pulse-green"></span> Last updated: ${time} <span class="badge-tag live">live</span>`;
        } else {
            badge.innerHTML = `<span class="pulse-yellow"></span> Last updated: ${time} <span class="badge-tag cached">cached</span>`;
        }
    }

    /** Haritadaki incident marker'larını temizle ve yeniden çiz */
    let incidentMarkerLayer = [];
    function redrawIncidentMarkers() {
        incidentMarkerLayer.forEach(m => map.removeLayer(m));
        incidentMarkerLayer = [];
        initMapIncidents();
    }
     ...
     * Önce live, başarısız olursa snapshot.
     */
    async function loadIncidentData() {
        ...
        // ── LIVE ATTEMPT ──
        try {
            const res = await fetch(CORS_PROXY + encodeURIComponent(IBB_CKAN_URL), {
            ...
            const zones = records.map(mapCKANRecord).filter(Boolean);
            if (zones.length === 0) throw new Error('Empty result');

            INCIDENT_ZONES = zones;
            setDataSourceBadge(true);
            redrawIncidentMarkers();
            console.log(`[IncidentPipeline] Live: ${zones.length} incident loaded.`);
            return;
        } catch (liveErr) {
            console.warn('[IncidentPipeline] Live fetch failed:', liveErr.message);
        }
        ...
        try {
            ...
            const json = await res.json();
            INCIDENT_ZONES = json.incidents || [];

            // snapshot_date'i HH:MM formatına çevir
            const snapDate = new Date(json.snapshot_date);
            const snapTime = snapDate.toLocaleString('tr-TR', {
                day: '2-digit', month: '2-digit',
                hour: '2-digit', minute: '2-digit'
            });
            setDataSourceBadge(false, snapTime);
            redrawIncidentMarkers();
            console.log(`[IncidentPipeline] Cached snapshot loaded (${json.snapshot_date}).`);
        } catch (snapErr) {
            console.error('[IncidentPipeline] Snapshot also failed:', snapErr.message);
            ...
            setDataSourceBadge(false, 'offline');
            redrawIncidentMarkers();
        }
    }
